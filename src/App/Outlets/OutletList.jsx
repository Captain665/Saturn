            ...
            <div className="flex flex-col w-full">
                ...
                <div className="self-center w-11/12 md:w-4/5 grid md:grid-cols-4 gap-2 md:gap-y-20 gap-y-10 p-1 md:mt-10 mt-5">
                    {outletData?.map(outlet => (
                        <div className="flex flex-col bg-white h-72 m-1 cursor-pointer hover:p-2 hover:delay-100" key={outlet.id} onClick={() => handleOnClick(outlet)}>
                            <ul className="h-3/4">
                                <img src={outlet.logoImage} alt="" className="object-cover w-full h-full object-center rounded-2xl" />
                                {outlet.deliveryCost === 0 && <li className="z-50 -mt-10 text-white font-extrabold text-center text-4xl">Free Delivery</li>}
                            </ul>
